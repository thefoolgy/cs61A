...
def reverse_recursive(lst):
    ...
    if n == 0:
        ...
        newlst.append(lst[-1])
        return newlst + reverse_recursive(lst[:n-1])
...
def distance(city_a, city_b):
    ...
    return sqrt((lat_b-lat_a)**2 + (lon_b-lon_a)**2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    shanghai = make_city('shanghai', lat, lon)
    distance_a_to_s = distance(shanghai, city_a)
    distance_b_to_s = distance(shanghai, city_b)
    if distance_a_to_s > distance_b_to_s:
...
def add_chars(w1, w2):
    ...
    if len(w1) == 0:
        return w2
    if w1[n2] == w2[n2]:
        return add_chars(w1[n2+1:],w2[n2+1:])
    else:
        return w2[0] + add_chars(w1,w2[n2+1:])
#pku solution:    
#   if len(w1) == 0:
#       return w2
#   if w1[0] == w2[0]:
