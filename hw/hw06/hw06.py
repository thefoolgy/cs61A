...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0 and self.balance == 0:
            ...
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(self.balance)
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            ...
            return min(bst_min(t.branches[1]),t.label)
    if t.is_leaf():
        return True
    elif len(t.branches) == 1:
        left, right = t.branches[0], t.branches[0]
        if t.label > left.label:
        ...
        if t.label < bst_max(left):
            return False
        elif t.label > bst_min(right):
            return False
        else:
